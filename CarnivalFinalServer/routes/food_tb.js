...
//selecting all the data of food table
...
app.get('/images', (request, response) => {
    fs.readdir('uploads', (error, files) => {
      if (error) {
        return response.status(500).send('Error reading the directory.');
      }
  ...
  app.get('/images/:imageName', (req, response) => {
...
//     });
//   });
app.post('/addfood', upload.single('image'), (request, response) => {
  try {
      const { food_id, food_name, food_price, food_type } = request.body;
      ...
      connection.query(query, values, (error, result) => {
          ...
          const foodId = result.insertId;
          response.status(201).json({ success: true, message: 'Food item created successfully', foodId });
      });
  } catch (error) {
...
//update 
  app.put('/food_tb/:id', upload.single('image'), (request, response) => {
    const { food_name, food_price, food_type, image } = request.body;
    const food_id = req.params.food_id;
   
    const image1 = request.file ? request.file.filename : null;
 
    const query = 'update food_tb set food_name = ?, food_price=?, food_type=?, image=? where food_id = ?';
    const values = [food_name, food_price, food_type, image, food_id];
    ...
    connection.query(query, values, (error, result) => {
      ...
      response.status(201).json({ success: true, message: ' fooditem updated successfully', productId });
    });
  });
